...
//        document.body.appendChild(iframe);
      ...
      doDelete(ids){
        ...
        var _this = this;
        this.deleteRequest("/employee/basic/user/" + ids).then(resp=> {
      ...
      loadEmps(){
        var _this = this;
        this.tableLoading = true;
        this.getRequest("/employee/basic/user?page=" + this.currentPage + "&size=10&keywords=" + this.keywords + "&politicId=" + this.emp.politicId + "&nationId=" + this.emp.nationId + "&posId=" + this.emp.posId + "&jobLevelId=" + this.emp.jobLevelId + "&engageForm=" + this.emp.engageForm + "&departmentId=" + this.emp.departmentId + "&beginDateScope=" + this.beginDateScope).then(resp=> {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.emp.id) {
              //更新
              this.tableLoading = true;
                this.putRequest("/employee/basic/user", this.emp).then(resp=> {
                _this.tableLoading = false;
                if (resp && resp.status == 200) {
                  var data = resp.data;
                  _this.$message({type: data.status, message: data.msg});
              ...
              //添加
              this.tableLoading = true;
              this.postRequest("/employee/basic/user", this.emp).then(resp=> {
                _this.tableLoading = false;
                if (resp && resp.status == 200) {
                  var data = resp.data;
                  _this.$message({type: data.status, message: data.msg});
...
//        delete this.user.jobLevel;
//        delete this.user.position;
//        delete this.user.nation;
//        delete this.user.politicsStatus;
